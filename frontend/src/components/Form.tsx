...
interface FormProps {
    ...
    images: File[];
    mensaje: { msg: string, type: string };
    loader: boolean;
}


export default function Form({ handleSubmit, handleImages, images, mensaje, loader }: FormProps) {
    return (

        <>
            {/** Poner el mensaje arriba a la derecha */}
            <form onSubmit={handleSubmit} className=" md:w-1/2 mx-auto shadow-xl rounded px-8 pt-6 pb-8 mb-4 flex flex-col my-2">

                {mensaje.msg !== "" && mensaje.type === "error" && (
                    <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-5" role="alert">
                        <strong className="font-bold">Error!</strong>
                        <span className="block sm:inline"> {mensaje.msg}</span>
                    </div>
                )}
